...
"""
CALCULADORA - ÍNDICE DE COMORBIDADE DE CHARLSON AJUSTADO POR IDADE (ICC-I)
...
#calcular ICC-I
def calcICC_I(v_icci,idade):
    #calcular peso-idade
    if (idade>= 50 and idade<=59):
        v_icci=v_icci+1
    if (idade>=60 and idade<=69):
        v_icci=v_icci+2
    if (idade>=70 and idade<=79):
        v_icci=v_icci+3
    if (idade>=80 and idade<=89):
        v_icci=v_icci+4
    if (idade>=90):
        v_icci=v_icci+5
    return v_icci
...
layout = [[sg.Text("Digite os códigos CID-10, separados por vírgulas, das comorbidades do paciente")],
          ...
          [sg.Input(key='-INPUT-')],
          [sg.Text("Digite a idade do paciente")],
          [sg.Input(key='-INPUT2-')],
          [sg.Text(size=(60,1), key='-OUTPUT-')],
          [sg.Button('Consultar'), sg.Button('Sair')]]

window = sg.Window('Calculadora ICC-I', layout)
...
while True:
    ...
    if(values['-INPUT-']=='' or values['-INPUT2-']==''):
        ...
        icci = calcICC_I(icc, int(values['-INPUT2-']))
        if(icci==0):
            window['-OUTPUT-'].update("Valor ICC-I: "+str(icci)+". As comorbidades e idade do paciente não agravam as chances de óbito.")
        else:    
            window['-OUTPUT-'].update("Valor ICC-I: "+str(icci)+". O paciente tem "+str(icci)+" vez(es) mais chance(s) de ir a óbito.")
